...
export default async function Image({ params }: { params: Promise<{ token: string }> }) {
  ...
  const circleUrl = `https://suwasarana.lk/circle/${token}`;
  const qrBase64 = await getQrCodeBase64(circleUrl);
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
            <div style={{ fontSize: '30px', color: '#fca5a5', fontWeight: '600', letterSpacing: '2px', textTransform: 'uppercase' }}>
              Private Circle Volunteer Request
            </div>
        ...
        {/* HOSPITAL INFO */}
        <div style={{ display: 'flex', alignItems: 'center', padding: '0 80px 24px 80px', gap: '16px' }}>
        ...
        {/* PILLS ROW */}
        <div style={{ display: 'flex', gap: '16px', padding: '0 80px 36px 80px' }}>
          {[
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: '26px', color: '#e5e7eb' }}>Your donation can save a life today.</div>
              <div style={{ fontSize: '30px', fontWeight: '900', color: '#ef4444' }}>Volunteer now →</div>
            </div>
          </div>
          {qrBase64 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
              ...
              <div style={{ fontSize: '16px', color: '#9ca3af' }}>Scan to view & share</div>
            </div>
          )}
        </div>
